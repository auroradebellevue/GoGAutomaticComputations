...
def make_gm(gog, travel_info, file_dir, kbmag_ftn_dir, out_dir, verbose):
    ...
    while(Mults_Same_NF == False):
        ...
        for iLetter in range(len(gm_states.label_numbers_pairs)):
            ...
            """
            while False in test_state_done:
                new_row=[]
                index = test_state_done.index(False)
                #print("State list", test_state_list)
                #print("Accepting State List", test_acc_states)
                for iFirstLetter in range(len(eps_alpha)):
                    new_s=set()
                    #for the letters starting with a letter and not _
                    for iSecondLetter in range(len(eps_alpha)):
                        alpha_index = iFirstLetter*len(eps_alpha)+iSecondLetter
                        #print("alphabet index", alpha_index, " max index", len(eps_alpha)*len(eps_alpha)-1)
                        if alpha_index != len(eps_alpha)*len(eps_alpha)-1: 
                            for iState in test_state_list[index]:
                                
                                temp_state = new_t[iState-1][iFirstLetter*len(eps_alpha)+iSecondLetter]
                                if verbose == True:
                                    g.write("new s for first letter: " +str(iFirstLetter)+" and second letter "+str(iSecondLetter)+"\t")
                                    g.write("index check: "+str(iFirstLetter*len(eps_alpha)+iSecondLetter)+"\n")
                                if temp_state != 0:
                                    new_s.add(temp_state)
                                    if {temp_state} not in test_state_list:
                                        test_state_list.append({temp_state})
                                        test_state_done.append(False)
                                        if temp_state in acceptingForLetter:
                                            test_acc_states.append({temp_state})
                                            print("adding to accept state list"+ str({temp_state}))
                                    
                    
                    if verbose == True:
                        g.write("current state ")
                        g.write(str(test_state_list[index])+"\t")
                        g.write("letter ")
                        g.write(str(eps_alpha[iFirstLetter])+"\t")
                        g.write("next state ")
                        g.write(str(new_s)+"\n")
                    ...
                    for state in test_state_list:
                        if new_s == state:
                            in_list = True
                            state_index = test_state_list.index(state)
                    if in_list == False:
                        test_state_list.append(new_s)
                        test_state_done.append(False)
                        state_index = len(test_state_list)-1
                        #if every state is an accept state, add it to the list 
                        isAcceptState = True
                        for st in new_s:
                            isAcceptState = isAcceptState and st in acceptingForLetter
                        if isAcceptState == True and len(new_s)!=0:
                            print("adding to accept state list"+ str(new_s))
                            test_acc_states.append(new_s)
                
                test_table.append(new_row)
                test_state_done[index]=True
                #end of while loop for building the projection
            #edit the format to be deterministic numberings
            
            #pop fail state
            fail_index=test_state_list.index(set())
            test_state_list.pop(fail_index)
            test_table.pop(fail_index)
            """
            test_table=[]
            ...
            """
            test_acc_lbl_sets = []
            for iRow in range(len(test_table)):
                for iEntry in range(len(test_table[iRow])):
                    table_entry = test_table[iRow][iEntry] #should be a set with a number that's too large
                    #print("table entry", table_entry)
                    new_st_as_set = set()
                    if table_entry in test_state_list:
                        new_st_as_set.add(test_state_list.index(table_entry)+1)                       
                        if table_entry in test_acc_states:
                            if new_st_as_set not in test_acc_lbl_sets:
                                test_acc_lbl_sets.append(new_st_as_set)
                    else:
                        new_st_as_set.add(0)
                    test_table[iRow][iEntry] = new_st_as_set
            """
        ...
        break
        #end of while loop for checking k              
    g.close()
...
def fancy_states(state_list, accept_list):
    print("hi")